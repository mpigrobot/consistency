% Demonstrate consistency of linear estimator.Tune the values of noise
...
% figure('Name', '过程噪声','color','w');
figure('Name', '观测噪声','color','w');
hold on; box on;
subplot(2,2,[1,3])
h = plot(s, meanY, 'g--', s, meanY1, 'c-.',s, meanY2, 'r:',s, meanY3, 'y--',...
    s, meanY4, 'm-.', s, chiUp, 'b-', s, chiLow, 'b-');
set(h, 'linewidth', width);
% legend('0.1*trueNoise','0.2*trueNoise','trueNoise','20*trueNoise',...
%     '50*trueNoise','95%置信区间');% different process noise levels
legend('0.7*trueNoise','0.8*trueNoise','trueNoise','2*trueNoise',...
    '10*trueNoise','95%置信区间');% different observation noise levels
xlim([min(s), max(s)]);
xlabel('时间(s)');
...
% legend('0.1*trueNoise','95%置信区间');% under-estimate process noise level
legend('0.7*trueNoise','95%置信区间');% under-estimate observation noise level
xlim([min(ss), max(ss)]);
xlabel('时间(s)');
ylabel('归一化自相关函数');
...
% legend('50*trueNoise','95%置信区间');% over-estimate process noise level
legend('10*trueNoise','95%置信区间');% over-estimate observation noise level
xlim([min(ss), max(ss)]);
xlabel('时间(s)');
ylabel('归一化自相关函数');
%% plot figure of NIS(normalized innovations square) use exactly matched process and observation noise levels
figure('Name', '真实噪声一致性检测','color','w');
hold on; box on;
subplot(2,1,1)
